...
class HiveLogger:
    """
    Logs swarm actions for the Hive and hashes logs for immutability.
    Thread-safe and configurable via environment variables.
    """
    def __init__(self, log_path: Optional[str] = None):
        self.log_path = log_path or os.getenv("HIVE_LOG", "logs/hive_swarm.jsonl")
        self._lock = threading.Lock()
        os.makedirs(os.path.dirname(self.log_path), exist_ok=True)

    def log(self, action: str, data: Dict[str, Any], extra: Optional[Dict[str, Any]] = None) -> str:
        """
        Logs a swarm action/decision.
        Args:
